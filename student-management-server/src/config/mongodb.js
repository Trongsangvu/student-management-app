...
class MongoDBConnectionManager {
  ...
  setupEventListeners() {
    ...
    mongoose.connection.on("connected", () => {
      ...
      console.log(
        chalk.green("✓ [MongoDB] Connection established successfully")
      );
    });
  ...
  async connect(uri = CONSTANTS.MONGO_URI, options = {}) {
    try {
      ...
      console.log(
        chalk.blue(
          `🔄 [MongoDB] Connecting... (Attempt ${this.connectionAttempts}/${this.maxRetries})`
        )
      );
      ...
      if (this.connectionAttempts >= this.maxRetries) {
        console.error(
          chalk.red("✗ [MongoDB] Max connection attempts reached. Giving up.")
        );
        throw new Error(
  ...
  scheduleReconnection() {
    if (this.connectionAttempts < this.maxRetries) {
      ...
      console.log(
        chalk.yellow(
          `⏳ [MongoDB] Retrying connection in ${delay / 1000} seconds...`
        )
      );
  ...
  async disconnect(force = false) {
    try {
      ...
      console.error(
        chalk.red("✗ [MongoDB] Error during disconnect:", error.message)
      );
      throw error;
    }
  }
